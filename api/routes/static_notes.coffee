...
router.post '/', server.loadUser, (req, res, next) ->
  ...
  note.score = 0
  note.user_email = req.user.email
...
router.get '/of_course/:courseId', server.loadUser, (req, res, next) ->
  Note.find(course: req.params.courseId).sort('-created_at').exec (err, notes) ->
    if err then next err
      ...
      else if not notes[0].course in req.user.admin_of
        for note in notes
          note.user = null
        res.send static_notes: notes
      else
        Note.find(course: req.params.courseId).populate('user').sort('-created_at').exec (err, notes) ->
          if err then next err
          else
            console.log notes
            res.send static_notes: notes
